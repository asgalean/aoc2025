% MATLAB solution for Part 2
% Run this in MATLAB

function total = solve_all_cases()
    % Test cases
    tests = {
    ...
    for i = 1:length(tests)
        result = solve_case(tests{i});
        fprintf('Test %d: %d presses\n', i, result);
        total = total + result;
    end
...
function presses = solve_case(line)
    % Parse the input
    [matrix, target] = parse_input(line);
    
    % Objective: minimize sum of all button presses
    % min c'*x where c = [1, 1, 1, ..., 1]
    n = size(matrix, 2);  % number of buttons
    c = ones(n, 1);
    
    % Equality constraints: matrix * x = target
    Aeq = matrix;
    beq = target;
    
    % Bounds: x >= 0 (all non-negative)
    lb = zeros(n, 1);
    ub = [];  % no upper bound
    ...
    % Solve using intlinprog (integer linear programming)
    options = optimoptions('intlinprog', 'Display', 'off');
    x = intlinprog(c, intcon, [], [], Aeq, beq, lb, ub, options);
    
    % Return total presses
    presses = sum(x);
end

function [matrix, target] = parse_input(line)
    % Extract target values from {...}
    target_str = regexp(line, '\{([0-9,]+)\}', 'tokens');
    target = str2double(strsplit(target_str{1}{1}, ','));
    num_counters = length(target);
    
    % Extract button effects from (...)
    buttons = regexp(line, '\(([0-9,]+)\)', 'tokens');
    num_buttons = length(buttons);
    
    % Build matrix
    matrix = zeros(num_counters, num_buttons);
    for btn = 1:num_buttons
        counters = str2double(strsplit(buttons{btn}{1}, ','));
        for cnt = counters
            matrix(cnt+1, btn) = 1;  % MATLAB is 1-indexed!
        end
    end
