...
typedef struct s_config
{
	...
	int *button_value_array; //esto va a haber que mallocearlo
...
/*
	Mash_limit es el numero limite que quiero pulsar cada boton, eso va a ser
	el numero qe voy a usar de base, si mi limite es 5 pulsadas, entonces voy a manejar 
	numeros en base 5, si mi limite son 10, entonces los numros van en base 10.
	cada boton es un digito de ese numero, ejemplo, si tenemos base 10:
	el numero 1489 me indica que el boton 0 lo pulso 9 veces, el 1 lo puulso 8, el 2 
	lo pulso 4 y el 4 lo pulso 1 vez, asi voy manejando hasta (base elevado a numero de botones)
	base = mash limit;
*/

int button_mash(t_config config, int mash_limit) 
{
	int limit = power(mash_limit, config.button_count);
	
	int i = 0;
	int button_press[config.button_count]; //array en el que guardo cuantas veces se pulsa cada boton
	int total = 1000000;
	while (i < limit)
	{
		int j = 0;
		int temp = i;
		int obj_temp = config.objective;
		while (j < config.button_count)
		{
			button_press[j] = temp % mash_limit;
			temp /= mash_limit;
			
			int h = 0;
			while (h < button_press[j])
			{
				//printf("Pressing button %d: %d XOR %d ", j, obj_temp, config.button_value_array[j]);
				obj_temp = obj_temp ^ config.button_value_array[j];
				//printf(" = %d\n", obj_temp);
				h++; 
			}
			j++;
		...
		if (obj_temp == 0)
		{
			int h = 0;
			int new = 0;
			while (h < config.button_count)
			{
				new += button_press[h];
				h++;
			}
			//printf("Found combination, times pressed: %d\n", new);
			if (new < total)
				total = new;
		}
		i++;
...
int main()
{
	...
	while (input[a])
	{
		...
		config.button_value_array = (int *)malloc(sizeof(int) * config.button_count);
		i = 0;
		int b_pos = 0;
		config.button_value_array[b_pos] = 0;
		while (input[a][i])
		{
			if (input[a][i] == '(')
			{
				while (input[a][i] != ')')
				{
					if (input[a][i] <= '9' && input[a][i] >= '0')
						config.button_value_array[b_pos] += power2(input[a][i] - '0');
					i++;
				}
				b_pos++;
				if (b_pos < config.button_count)
					config.button_value_array[b_pos] = 0;
			}
		...
		/*
		i = -1;
		while (++i < config.button_count)
		{
			printf("Button %d value: %d\n", i, config.button_value_array[i]);
		}
		*/
		total += button_mash(config, 4); //3 tambien sirve
		a++;
	}	
	printf("%d\n", total);
}
